...
function validateEditProfile() {
    ...
    if (firstname.length < 3 || firstname.length > 30) {
        alert("Độ dài firstname từ 3 - 30 ký tự.");
        document.getElementById('firstname').focus();
        return false;
    } else if (lastname.length < 3 || lastname.length > 30) {
        alert("Độ dài lastname từ 3 - 30 ký tự.");
        document.getElementById('lastname').focus();
        return false;
    } else if (phone.length < 9 || phone.length > 13) {
        alert("Độ dài phone từ 9 - 13 ký tự.");
        document.getElementById('phone').focus();
        return false;
    } else if (description.length > 200) {
        alert("Độ dài description nhỏ hơn 200 ký tự.");
